...
<title>Follow Users and Artists</title>
<center>Users you may want to follow...</center>
...
<body>
<form name="followUserArtistForm" method="post" action="FollowUserSignUpController">
<%List<List<UserBean>> reccomUserList= (List<List<UserBean>>)request.getAttribute("reccoUserList"); 
	...
	<%if(null!=reccomUserList && !reccomUserList.isEmpty()){ %>
		<table align="center" border=1 width=50% id="tfhover" class="tftable">
			...
			<%for(List<UserBean> userList:reccomUserList){ 
				for(UserBean userBean: userList){%>
					<tr>
						<td><input type="checkbox" name="reccomUserSelected" value=<%=userBean.getUserid() %>></td>
	...
	<%}else{ %>
		<center>Please click on continue to follow recommended artist</center>
	<%} %>
