...
import com.MusicSystem.Connection.DBManager;
import com.MusicSystem.ExceptionLogger.ErrorLog;
import com.MusicSystem.bean.ConcertDetailsBean;

public class ViewYourRecommdDAO {
	public List<ConcertDetailsBean> viewSelfConcertReccomend(String userId) throws ConnectionFailureException, SQLException, IOException{
		Connection conn=DBManager.getConnection();
		CallableStatement callableStatement=conn.prepareCall("call VIEW_SELF_CONCERT_RECCO_PROC(?,?,?)");
		callableStatement.registerOutParameter(2, OracleTypes.CURSOR);
		callableStatement.registerOutParameter(3, java.sql.Types.VARCHAR);
		callableStatement.setString(1, userId);
		callableStatement.execute();
		ResultSet rs=((OracleCallableStatement)callableStatement).getCursor(2);
		List<ConcertDetailsBean> selfRecomendConcertList=new ArrayList<ConcertDetailsBean>();
		if(!callableStatement.wasNull()){
			while(rs.next()){
				...
				selfRecomendConcertList.add(concertDetailsBean);
			}
		}
		...
		return selfRecomendConcertList;
	}
	
	public void deleteSelfRecommendation(String userId, int concertId) throws ConnectionFailureException, SQLException, IOException{
		Connection conn=DBManager.getConnection();
		CallableStatement callableStatement=conn.prepareCall("call DELETE_RECOMMENDATION_PROC(?,?,?)");
		callableStatement.registerOutParameter(3, java.sql.Types.VARCHAR);
		callableStatement.setString(1, userId);
